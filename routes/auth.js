...
// Admin session cookie — separate name from COOKIE_OPTIONS above so an
// admin session never collides with a customer session in the same browser.
...
// ── Admin session (separate cookie from customer session) ──
...
// GET /api/auth/admin-me — check if the admin session is still valid
